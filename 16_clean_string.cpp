...
string clean_string(string, bool = false, vector <char> = {});
...
int main()
{
    ...
    cout << clean_string("Elzero//''00Web@#$School__--") << endl;
    cout << clean_string("Elzero//''00Web@#$School__--", true) << endl;
    cout << clean_string("Elzero//''00Web@#$School__--", true, {'-', '$'}) << endl;
...
string clean_string(string s, bool keepNums, vector <char> keep) {
    string clean = "";

    for (auto c : s) {
        if (isalpha(c) || c == '_' || (keepNums && isdigit(c)))
            clean += c;
        else {
            for (auto it : keep) {
                if (c == it)
